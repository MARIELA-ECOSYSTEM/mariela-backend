...
describe("HTTP — Coleções (integração — servidor real)", () => {
  ...
  // Etapa 18.10 — mass assignment: `codigo`, `produtosVinculados` e o soft
  // delete são campos internos/calculados (nunca fazem parte de
  // `CriarColecaoDto`). O whitelist global (`forbidNonWhitelisted: true`)
  // deve rejeitar o payload inteiro (400) em vez de simplesmente ignorar os
  // campos extras.
  it("POST /api/v1/colecoes com campos internos/calculados no payload é rejeitado (400) pelo whitelist global — nunca usados como autoridade", async () => {
    const resposta = await fetch(`${baseUrl}/api/v1/colecoes`, {
      method: "POST",
      headers: authHeaders(),
      body: JSON.stringify({
        nome: "Coleção Mass Assignment",
        inicio: "2026-01-01",
        fim: "2026-03-31",
        codigo: "COL-9999",
